...
ENV VIRTUAL_ENV /opt/mkdocs-strict-jinja2-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/mkdocs-strict-jinja2/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/mkdocs-strict-jinja2/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/mkdocs_strict_jinja2/ && touch src/mkdocs_strict_jinja2/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/mkdocs-strict-jinja2/
